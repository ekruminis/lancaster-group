...
public class Game {
    ...
    public void run(int width, int height) {
        ...
        x = new Enemy (900,600,player,"1");
        Thankyou = new Enemy(800,730,player,"2");
        enemies.add(x);
        enemies.add(Thankyou);
        ArrayList<Projectiles> rocks = new ArrayList<>(1);
        ...
        while (true) {
            ...
            if (Keyboard.isKeyPressed (Keyboard.Key.LBRACKET) && dropped == true) {
                dropped = false;
                rocks.add(0, new Projectiles (getHero().getCenterX(),getHero().getCenterY(),player, false, window));
                shooted = true;

            }
            if (Keyboard.isKeyPressed (Keyboard.Key.RBRACKET) && dropped == true) {
                dropped = false;
                rocks.add(0, new Projectiles (getHero().getCenterX(),getHero().getCenterY(),player, true, window));
                shooted = true;
